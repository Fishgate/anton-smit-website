...
class Owlabkbs_Common {
    ...
    public function get_slider_themes() {
        ...
        $themes = array(
            array(
                'value' => 'base',
                'name'  => __( 'Base', 'owlabkbs' ),
                'file'  => $this->base->file
            ),
            array(
                'value' => 'classic',
                'name'  => __( 'Classic', 'owlabkbs' ),
                'file'  => $this->base->file
            )
        );

        return apply_filters( 'owlabkbs_slider_themes', $themes );
    ...
    public function get_slider_transitions() {
        ...
        $transitions = array(
            array(
                'value' => 'fade',
                'name'  => __( 'Fade', 'owlabkbs' )
            )
        );

        return apply_filters( 'owlabkbs_slider_transitions', $transitions );
    ...
    public function get_config_defaults( $post_id ) {
        ...
        return apply_filters( 'owlabkbs_defaults', $defaults, $post_id );
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = true, $align = 'c', $quality = 100, $retina = false ) {
        ...
        if ( ! file_exists( $dest_file_name ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'owlabkbs-error-no-editor', __( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', 'owlabkbs' ) );
            }
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'owlabkbs-error-no-url', __( 'No image URL specified for cropping.', 'owlabkbs' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'owlabkbs-error-no-file', __( 'No file could be found for the image URL specified.', 'owlabkbs' ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'owlabkbs-error-no-size', __( 'The dimensions of the original image could not be retrieved for cropping.', 'owlabkbs' ) );
        }
    ...
    public function flush_slider_caches( $post_id, $slug = '' ) {
        ...
        do_action( 'owlabkbs_flush_caches', $post_id, $slug );
     ...
     * @since 1.0.0
     *
     * @return object The Owlabkbs_Common object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Owlabkbs_Common ) ) {
            self::$instance = new Owlabkbs_Common();
        }
...
$owlabkbs_Common = Owlabkbs_Common::get_instance();