...
class Owlabkbs_Media {
    ...
    public function media_upload_form() {
        ...
        if ( ! $this->device_can_upload() ) {
            echo '<p>' . sprintf( __( 'The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.', 'owlabkbs' ), 'http://wordpress.org/mobile/' ) . '</p>';
            return;
        }
    ...
    public function do_media_upload_form( $width, $height ) {
        ...
        <div id="owlabkbs-upload-error"></div>
        <div id="owlabkbs-plupload-upload-ui" class="hide-if-no-js">
            <?php do_action( 'pre-plupload-upload-ui' ); ?>
            <div id="owlabkbs-drag-drop-area">
                <div class="drag-drop-inside">
                    <p class="drag-drop-info"><?php _e( 'Drop images here', 'owlabkbs' ); ?></p>
                    <p><?php _ex( 'or', 'Uploader: Drop images here - or - Select Images', 'owlabkbs' ); ?></p>
                    <p class="drag-drop-buttons">
                        <input id="owlabkbs-plupload-browse-button" type="button" value="<?php esc_attr_e( 'Select Images', 'owlabkbs' ); ?>" class="button" />
                    </p>
                </div>
            </div>
        ...
        <div id="owlabkbs-html-upload-ui" class="hide-if-js">
            <?php do_action('pre-html-upload-ui'); ?>
            <p id="owlabkbs-async-upload-wrap">
                <label class="screen-reader-text" for="owlabkbs-async-upload"><?php _e( 'Upload', 'owlabkbs' ); ?></label>
                <input type="file" name="async-upload" id="owlabkbs-async-upload" />
                <?php submit_button( __( 'Upload', 'owlabkbs' ), 'button', 'html-upload', false ); ?>
                <a href="#" onclick="try{top.tb_remove();}catch(e){};return false;"><?php _e( 'Cancel', 'owlabkbs' ); ?></a>
            </p>
            <div class="clear"></div>
            <?php do_action( 'post-html-upload-ui' ); ?>
        </div>

        <span class="max-upload-size"><?php printf( __( 'Maximum upload file size: %d%s.', 'owlabkbs' ), esc_html( $this->get_upload_size_unit( $sizes ) ), esc_html( $sizes[$this->get_upload_size_unit( $sizes, 'unit' )] ) ); ?></span>
        <?php
        ...
        if ( ( $is_IE || $is_opera ) && $max_upload_size > 100 * 1024 * 1024 ) {
            echo '<span class="big-file-warning">' . __( 'Your browser has some limitations uploading large files with the multi-file uploader. Please use the browser uploader for files over 100MB.', 'owlabkbs' ) . '</span>';
        }
     ...
     * @since 1.0.0
     *
     * @return object The Owlabkbs_Media object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Owlabkbs_Media ) ) {
            self::$instance = new Owlabkbs_Media();
        }
...
$owlabkbs_Media = Owlabkbs_Media::get_instance();